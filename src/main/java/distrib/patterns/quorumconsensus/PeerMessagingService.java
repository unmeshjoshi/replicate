...
class PeerMessagingService {
    ...
    private void handleGetVersionRequest(RequestOrResponse request) {
        try {
            ...
            StoredValue storedValue = kvStore.get(getVersionRequest.getKey());
            MonotonicId version = (storedValue == null) ? MonotonicId.empty() : storedValue.getVersion();
            byte[] serialize = JsonSerDes.serialize(version);
            sendResponseMessage(new RequestOrResponse(request.getGeneration(), RequestId.GetVersionResponse.getId(),
                    serialize, request.getCorrelationId(), peerConnectionAddress), request.getFromAddress());
        } catch (Exception e) {
            e.printStackTrace();;
        }
    }
